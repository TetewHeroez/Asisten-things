...
\setminted[java]{bgcolor=bg,fontsize=\small,frame=single,bgcolorpadding=1mm,escapeinside=||,linenos,numbersep=5pt}
...
\begin{document}
  ...
  \begin{enumerate}
    \item Sebutkan dan jelaskan macam-macam \textit{exception} di Java yang kamu ketahui!
    \item Perhatikan potongan program berikut:
    \begin{minted}{java}
  int[] a = new int[3];
  try {
    //statement 1
    ...
    System.err.print("Padamu ");
  } catch (ArrayIndexOutOfBoundsException e) {
    System.err.print("Himpunan ");
  } catch (Exception e) {
    System.err.print("24 ");
  } finally {
    ...
    \end{minted}
    Anda diminta untuk membuat error pada baris \texttt{statement 1} atau \texttt{statement 2} agar program di atas dapat menampilkan output berdasarkan \texttt{catch} yang sesuai dengan kondisi sub-soal berikut:
    \begin{enumerate}
      \item Program menampilkan \texttt{Padamu Matematika}
      \item Program menampilkan \texttt{Himpunan Matematika}
      \item Bisakah program menampilkan \texttt{Padamu Himpunan Matematika}? Jelaskan!
    \end{enumerate}
  \end{enumerate}
